...
class DefaultColor:
    ...
    REPO_CLEAN_BG = 113  # a light green color
    REPO_CLEAN_FG = 0  # black
    REPO_DIRTY_BG = 131  # indian red
    REPO_DIRTY_FG = 15  # white
...
def add_repo_name():
	output = subprocess.Popen(['git', 'rev-parse', '--show-toplevel'],
		env={"LANG": "C", "HOME": os.getenv("HOME")}, stdout=subprocess.PIPE).communicate()[0]
	repo = output.split('/')[-1].strip()
	powerline.append(' %s ' % repo, Color.CWD_FG, Color.PATH_BG)
